...
public class MyDrawerScrollView extends ScrollView implements Handler.Callback, IWXScroller,
    ...
    private MyDrawerScrollComponent mWAScroller;
    //sticky
    ...
    public MyDrawerScrollView(Context context) {
        super(context);
    ...
    public MyDrawerScrollView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public MyDrawerScrollView(Context context, AttributeSet attrs, int defStyle) {
     ...
     */
    public void addScrollViewListener(MyDrawerScrollView.WXScrollViewListener scrollViewListener) {
        if (!mScrollViewListeners.contains(scrollViewListener)) {
            mScrollViewListeners.add(scrollViewListener);
        }
    }

    public void removeScrollViewListener(com.taobao.weex.ui.view.WXScrollView.WXScrollViewListener scrollViewListener) {
    ...
    protected void onScrollChanged(int x, int y, int oldx, int oldy) {
        mScrollX = getScrollX();
        mScrollY = getScrollY();
        onScroll(MyDrawerScrollView.this, mScrollX, mScrollY);
    ...
    protected void onScroll(MyDrawerScrollView scrollView, int x, int y) {
        int count = mScrollViewListeners == null ? 0 : mScrollViewListeners.size();
        for (int i = 0; i < count; ++i) {
        ...
        switch (msg.what) {
            case 0:
                ...
                if (mInitialPosition - newPosition == 0) {//has stopped
                    onScrollStopped(MyDrawerScrollView.this, getScrollX(), getScrollY());
                } else {
                    onScroll(MyDrawerScrollView.this, getScrollX(), getScrollY());
                    mInitialPosition = getScrollY();
                    if (mScrollerTask != null) {
                        mScrollerTask.sendEmptyMessageDelayed(0, mCheckTime);
                    }
    ...
    protected void onScrollStopped(MyDrawerScrollView scrollView, int x, int y) {
        int count = mScrollViewListeners == null ? 0 : mScrollViewListeners.size();
        for (int i = 0; i < count; ++i) {
    ...
    public interface WXScrollViewListener {
        ...
        void onScrollChanged(MyDrawerScrollView scrollView, int x, int y, int oldx, int oldy);

        void onScrollToBottom(MyDrawerScrollView scrollView, int x, int y);

        void onScrollStopped(MyDrawerScrollView scrollView, int x, int y);

        void onScroll(MyDrawerScrollView scrollView, int x, int y);
    }

    public void setWAScroller(MyDrawerScrollComponent mWAScroller) {
