...
public class MyScrollView extends ScrollView implements Handler.Callback, IWXScroller,
    ...
    private List<MyScrollView.WXScrollViewListener> mScrollViewListeners;
    private MyScrollerComponent mWAScroller;
    //sticky
     ...
     */
    public void addScrollViewListener(MyScrollView.WXScrollViewListener scrollViewListener) {
    ...
    protected void onScrollChanged(int x, int y, int oldx, int oldy) {
        mScrollX = getScrollX();
        mScrollY = getScrollY();
        onScroll(MyScrollView.this, mScrollX, mScrollY);
    ...
    protected void onScroll(MyScrollView scrollView, int x, int y) {
        int count = mScrollViewListeners == null ? 0 : mScrollViewListeners.size();
        for (int i = 0; i < count; ++i) {
        ...
        switch (msg.what) {
            case 0:
                ...
                if (mInitialPosition - newPosition == 0) {//has stopped
                    onScrollStopped(MyScrollView.this, getScrollX(), getScrollY());
                } else {
                    onScroll(MyScrollView.this, getScrollX(), getScrollY());
                    mInitialPosition = getScrollY();
                    if (mScrollerTask != null) {
                        mScrollerTask.sendEmptyMessageDelayed(0, mCheckTime);
                    }
    ...
    protected void onScrollStopped(MyScrollView scrollView, int x, int y) {
        int count = mScrollViewListeners == null ? 0 : mScrollViewListeners.size();
        for (int i = 0; i < count; ++i) {
    ...
    public interface WXScrollViewListener {
        ...
        void onScrollToBottom(MyScrollView scrollView, int x, int y);

        void onScrollStopped(MyScrollView scrollView, int x, int y);

        void onScroll(MyScrollView scrollView, int x, int y);
    }

    public void setWAScroller(MyScrollerComponent mWAScroller) {
