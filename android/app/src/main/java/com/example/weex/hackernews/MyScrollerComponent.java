...
public class MyScrollerComponent extends WXVContainer<ViewGroup> implements MyScrollView.WXScrollViewListener,Scrollable {
    ...
    public MyScrollerComponent(WXSDKInstance instance, WXDomObject node,
                      WXVContainer parent) {
    ...
    protected ViewGroup initComponentHostView(@NonNull Context context) {
        ...
        if(("horizontal").equals(scroll)){
            ...
            MyBounceScrollerView scrollerView = new MyBounceScrollerView(context, mOrientation, this);
            mRealView = new FrameLayout(context);
            MyScrollView innerView = scrollerView.getInnerView();
            innerView.addScrollViewListener(this);
            FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(
                    FrameLayout.LayoutParams.MATCH_PARENT, FrameLayout.LayoutParams.MATCH_PARENT);
            innerView.addView(mRealView, layoutParams);
            innerView.setVerticalScrollBarEnabled(true);
            innerView.addScrollViewListener(new MyScrollView.WXScrollViewListener() {
                @Override
                ...
                @Override
                public void onScrollToBottom(MyScrollView scrollView, int x, int y) {

                }

                @Override
                public void onScrollStopped(MyScrollView scrollView, int x, int y) {
                ...
                @Override
                public void onScroll(MyScrollView scrollView, int x, int y) {
                    List<OnWXScrollListener> listeners = getInstance().getWXScrollListeners();
                    if(listeners!=null && listeners.size()>0){
                        for (OnWXScrollListener listener : listeners) {
                            if (listener != null) {
    ...
    public void scrollBy(final int x, final int y) {
        ...
        getInnerView().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if(mOrientation== Constants.Orientation.VERTICAL){
                    ((MyScrollView) getInnerView()).smoothScrollBy(0, y);
                }else{
     ...
     * @param scrollView  the MyScrollView
     * @param x the X direction
     * @param y the Y direction
     */
    protected void onLoadMore(MyScrollView scrollView, int x, int y) {
        try {
